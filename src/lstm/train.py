...
def create_sequences(df, feature_cols, le_multi, window_size):
    print(
        f"--> Đang nhóm luồng mạng (Groupby) và trượt cửa sổ cỡ {window_size}...")
...
def main():
    ...
    for i in range(1, 5):
        ...
        if os.path.exists(file_path):
            ...
            df_list.append(pd.read_csv(file_path, header=None,
                           names=config.UNSW_COLUMNS, low_memory=False))
        else:
            print(f"  [CẢNH BÁO] Không tìm thấy file {
                  file_path}. Đang bỏ qua...")

    if not df_list:
        print(
            "LỖI KHẨN CẤP: Không tìm thấy dữ liệu thô tại thư mục chỉ định trong config.py")
        return
    ...
    print(f"\n--- PHÂN CHIA DỮ LIỆU ({config.TRAIN_RATIO *
          100}% TRAIN / {(1-config.TRAIN_RATIO)*100}% TEST) ---")
    ...
    artifacts = {}
    X_train_raw, y_train_bin, y_train_cat, artifacts = process(
        df_train_raw, is_train=True, artifacts=artifacts)
    X_test_raw, y_test_bin, y_test_cat, _ = process(
        df_test_raw, is_train=False, artifacts=artifacts)
    ...
    selected_label, selected_cat = select(
        X_train_for_select, y_train_bin, y_train_cat)
    ...
    print(
        f"-> Số lượng đặc trưng tinh hoa được giữ lại: {len(union_features)} cột.")
    ...
    final_feature_cols = [c for c in X_train_encoded.columns if c not in [
        'srcip', 'dstip', 'label', 'attack_cat']]

    print("Đang tạo chuỗi dữ liệu 3D cho tập Train và Test...")
    X_train, y_train_bin_3d, y_train_mul_3d = create_sequences(
        X_train_encoded, final_feature_cols, le_multi, config.WINDOW_SIZE)
    X_test, y_test_bin_3d, y_test_mul_3d = create_sequences(
        X_test_encoded, final_feature_cols, le_multi, config.WINDOW_SIZE)
    ...
    del X_test, X_test_encoded, df_meta_train, df_meta_test
    gc.collect()
    ...
    model_tier1 = build_tier1_binary_model(
        config.WINDOW_SIZE, len(final_feature_cols))
    ...
    del X_train, y_train_bin_3d
    gc.collect()

    print("Đang kích hoạt SMOTE cân bằng nội bộ các nhóm tấn công...")
    samples, window, features = X_train_attacks.shape
    X_flat_attacks = X_train_attacks.reshape(
        (samples, window * features)).astype(np.float32)

    smote = SMOTE(sampling_strategy='not majority', random_state=42)
    X_res_flat_attacks, y_res_mul_attacks = smote.fit_resample(
        X_flat_attacks, y_train_mul_attacks)
    X_train_res_attacks = X_res_flat_attacks.reshape(
        (X_res_flat_attacks.shape[0], window, features))

    print(f"-> Kích thước tập dữ liệu Tầng 2 sau khi tăng cường SMOTE: {
          X_train_res_attacks.shape[0]} mẫu.")
    del X_train_attacks, X_flat_attacks
    gc.collect()

    model_tier2 = build_tier2_multiclass_model(
        config.WINDOW_SIZE, len(final_feature_cols), len(le_multi.classes_))
