...
LOG1P = ['sbytes', 'dbytes', 'spkts', 'dpkts', 'sloss',
         'dloss', 'sinpkt', 'dinpkt', 'sjit', 'djit', 'res_bdy_len']
FLAGS = ["is_ftp_login", "is_sm_ips_ports", "is_tcp", "is_ftp", "is_http",
         "tcp_seq_established", "tcp_seq_one_sided", "is_zero_dur", "is_short_flow", "zero_win"]
...
def process(df: pd.DataFrame, is_train: bool, artifacts: dict) -> tuple:
    ...
    if 'stcpb' in df.columns and 'dtcpb' in df.columns:
        df['tcp_seq_established'] = (
            (df['stcpb'] != 0) & (df['dtcpb'] != 0)).astype(int)
        df['tcp_seq_one_sided'] = (
            (df['stcpb'] != 0) ^ (df['dtcpb'] != 0)).astype(int)
    ...
    if 'rate' in df.columns:
        if is_train:
            corr, _ = spearmanr(
                df['rate'], (df['spkts'] + df['dpkts']) / (df['dur'] + 1e-9))
    ...
    # -----------------------------------------------------------------
    dangerous_cols = ['sport', 'dsport', 'res_bdy_len',
    ...
    df['is_zero_dur'] = (pd.to_numeric(
        df['dur'], errors='coerce') == 0).astype(int)
    df['is_short'] = ((pd.to_numeric(df['dur'], errors='coerce') > 0) & (
        pd.to_numeric(df['dur'], errors='coerce') < 0.001)).astype(int)
    df['dur'] = np.log1p(pd.to_numeric(
        df['dur'], errors='coerce').fillna(0).astype(float))

    math_cols = ['sjit', 'djit', 'sinpkt', 'dinpkt', 'synack', 'tcprtt',
                 'ackdat', 'swin', 'dwin', 'ct_srv_src', 'ct_src_ltm', 'ct_dst_src_ltm']
    ...
    if 'swin' in df.columns and 'dwin' in df.columns:
        ...
        df['win_asymmetry'] = np.abs(
            df['swin'] - df['dwin']) / (df['swin'] + df['dwin'] + 1)

    if 'ct_srv_src' in df.columns and 'ct_src_ltm' in df.columns:
        df['srv_diversity'] = 1.0 - \
            np.clip(df['ct_srv_src'] / (df['ct_src_ltm'] + 1e-9), 0.0, 1.0)
...
def select(X_train: pd.DataFrame, y_label: pd.Series, y_cat: pd.Series) -> tuple:
    ...
    y_cat_enc = pd.Series(le_ycat.fit_transform(
        y_cat.astype(str)), index=y_cat.index)
    ...
    sample_idx = np.random.choice(
        len(vif_arr), min(10000, len(vif_arr)), replace=False)
    ...
    mi_label = pd.Series(mutual_info_classif(
        X_train.iloc[:20000], y_label.iloc[:20000], random_state=42), index=features)
    mi_cat = pd.Series(mutual_info_classif(
        X_train.iloc[:20000], y_cat_enc.iloc[:20000], random_state=42), index=features)
    thresh_label = mi_label.quantile(0.1)
    thresh_cat = mi_cat.quantile(0.1)
    low_mi = set(mi_label[mi_label < thresh_label].index) & set(
        mi_cat[mi_cat < thresh_cat].index)
    for f in low_mi:
        flag(f)

    X_arr = X_train.values
    lgb_label = lgb.LGBMClassifier(
        n_estimators=100, random_state=42, n_jobs=-1, is_unbalance=True, verbose=-1)
    lgb_label.fit(X_arr, y_label)
    gain_label = pd.Series(lgb_label.booster_.feature_importance(
        importance_type='gain'), index=features)

    lgb_cat = lgb.LGBMClassifier(
        n_estimators=100, random_state=42, n_jobs=-1, class_weight='balanced', verbose=-1)
    lgb_cat.fit(X_arr, y_cat_enc)
    gain_cat = pd.Series(lgb_cat.booster_.feature_importance(
        importance_type='gain'), index=features)

    thresh_glabel = gain_label.quantile(0.05)
    thresh_gcat = gain_cat.quantile(0.05)
    low_gain = set(gain_label[gain_label < thresh_glabel].index) & set(
        gain_cat[gain_cat < thresh_gcat].index)
    for f in low_gain:
        flag(f)

    binary_scorer = make_scorer(recall_score, pos_label=1, zero_division=0)
    perm = permutation_importance(
        lgb_label, X_arr[:10000], y_label.iloc[:10000], scoring=binary_scorer, n_repeats=3, random_state=42, n_jobs=-1)
    ...
    sss1 = StratifiedShuffleSplit(
        n_splits=1, train_size=min(30000, len(X_arr)), random_state=42)
    idx1, _ = next(sss1.split(X_train[features_after], y_label))
    rfecv_label = RFECV(
        estimator=lgb.LGBMClassifier(
            n_estimators=50, random_state=42, n_jobs=-1, is_unbalance=True, verbose=-1),
        step=2,
    ...
    selected_label = [f for f, sel in zip(
        features_after, rfecv_label.support_) if sel]
    ...
    rfecv_cat = RFECV(
        estimator=lgb.LGBMClassifier(
            n_estimators=50, random_state=42, n_jobs=-1, class_weight='balanced', verbose=-1),
        step=2,
        cv=StratifiedKFold(n_splits=3, shuffle=True, random_state=42),
    ...
    rfecv_cat.fit(X_train[features_after_cat].values[idx2],
                  y_cat_enc.values[idx2])
    selected_cat = [f for f, sel in zip(
        features_after_cat, rfecv_cat.support_) if sel]
...
class SmoothedTargetEncoder(BaseEstimator, TransformerMixin):
    ...
    def _fit_single(self, col: pd.Series, target: np.ndarray, key):
        ...
        for val, grp in tmp.groupby('val'):
            ...
            enc_map[val] = (n * cat_mean + self.smoothing *
                            global_mean) / (n + self.smoothing)
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for key in self.classes_:
            ...
            col_name = f"proto_enc_{
                key}" if self.is_multiclass else "proto_enc"
...
class Pipeline(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y_label: pd.Series, y_cat: pd.Series):
        if 'proto' in X.columns:
            self.proto_enc_label_ = SmoothedTargetEncoder(
                'proto', smoothing=10, is_multiclass=False)
            self.proto_enc_label_.fit(X[['proto']], y_label)
            self.proto_enc_cat_ = SmoothedTargetEncoder(
                'proto', smoothing=10, is_multiclass=True)
        ...
        ohe_names = (self.ohe_.get_feature_names_out(
            self.ohe_cols_).tolist() if self.ohe_ is not None else [])
        non_scale_cols = set(FLAGS) | set(ohe_names)
        self.cont_cols_ = [
            c for c in X_encoded.columns if c not in non_scale_cols]
    ...
    def _apply_encodings(self, X: pd.DataFrame) -> pd.DataFrame:
        if 'proto' in X.columns and self.proto_enc_label_ is not None:
            ...
            X = pd.concat([X.drop(columns=['proto']),
                          proto_label, proto_cat], axis=1)

        if self.ohe_cols_ and self.ohe_ is not None:
            ohe_names = self.ohe_.get_feature_names_out(
                self.ohe_cols_).tolist()
            ohe_df = pd.DataFrame(self.ohe_.transform(
                X[self.ohe_cols_].astype(str)), columns=ohe_names, index=X.index)
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.cont_cols_:
            for col in self.cont_cols_:
                X_encoded[col] = X_encoded[col].apply(force_to_numeric)
            X_encoded[self.cont_cols_] = self.scaler_.transform(
                X_encoded[self.cont_cols_])
        return X_encoded
