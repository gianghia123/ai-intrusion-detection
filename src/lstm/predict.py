...
def main():
    ...
    print(classification_report(y_test_bin, y_pred_tier1,
    ...
    if len(attack_indices_test) > 0:
        ...
        preds_tier2 = model_tier2.predict(
            X_test_suspects, batch_size=config.BATCH_SIZE)
    ...
    print(classification_report(y_test_mul, final_predictions,
          labels=labels_idx, target_names=target_names, zero_division=0))
    ...
    print(f"✅ XUẤT FILE KẾT QUẢ BÁO CÁO THÀNH CÔNG TẠI: {
          os.path.abspath(config.OUTPUT_REPORT_PATH)}")
